...
public class CachedAttributesServiceDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Then calls {@link AttributeCaffeineCache#put(VersionedCacheKey, Serializable)}.
   * </ul>
   *
   * <p>Method under test: {@link CachedAttributesService#findAllKeysByEntityIds(TenantId, List,
   * String)}
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"List CachedAttributesService.findAllKeysByEntityIds(TenantId, List, String)"})
  public void testFindAllKeysByEntityIdsWithTenantIdEntityIdsScope_thenCallsPut() {
    // Arrange
    ...
    AttributeCaffeineCache cache = mock(AttributeCaffeineCache.class);
    doNothing().when(cache).put(Mockito.<AttributeCacheKey>any(), Mockito.<AttributeKvEntry>any());
    ...
    cache.put(attributeCacheKey, baseAttributeKvEntry);
    JpaExecutorService jpaExecutorService = new JpaExecutorService();

    CachedAttributesService cachedAttributesService =
        new CachedAttributesService(
            attributesDao, jpaExecutorService, statsFactory, new CacheExecutorService(), cache);
    ...
    // Assert
    verify(cache).put(isA(AttributeCacheKey.class), isA(AttributeKvEntry.class));
    verify(statsFactory, atLeast(1))
        .createDefaultCounter(eq("attributes.cache"), isA(String[].class));
    verify(attributesDao)
        .findAllKeysByEntityIdsAndAttributeType(isA(TenantId.class), isA(List.class), eq("Scope"));
  ...
  public void testFindAllKeysByEntityIdsWithTenantIdEntityIds_givenStatsFactory() {
    // Arrange
    when(attributesDao.findAllKeysByEntityIds(
            Mockito.<TenantId>any(), Mockito.<List<EntityId>>any()))
        .thenReturn(new ArrayList<>());
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"List CachedAttributesService.findAllKeysByEntityIds(TenantId, List)"})
  public void testFindAllKeysByEntityIdsWithTenantIdEntityIds_givenStatsFactory2() {
    // Arrange
    when(attributesDao.findAllKeysByEntityIds(
            Mockito.<TenantId>any(), Mockito.<List<EntityId>>any()))
        .thenReturn(new ArrayList<>());
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"List CachedAttributesService.findAllKeysByEntityIds(TenantId, List)"})
  public void testFindAllKeysByEntityIdsWithTenantIdEntityIds_thenCallsPut() {
    // Arrange
    AttributesDao attributesDao = mock(AttributesDao.class);
    when(attributesDao.findAllKeysByEntityIds(
            Mockito.<TenantId>any(), Mockito.<List<EntityId>>any()))
  ...
  public void testRemoveAllWithTenantIdEntityIdAttributeScopeList()
      throws InterruptedException, ExecutionException {
    // Arrange
  ...
  public void testRemoveAllWithTenantIdEntityIdAttributeScopeList_thenCallsPut()
      throws InterruptedException, ExecutionException {
    // Arrange
    DefaultStatsFactory statsFactory = mock(DefaultStatsFactory.class);
    AtomicInteger aiCounter = new AtomicInteger();
