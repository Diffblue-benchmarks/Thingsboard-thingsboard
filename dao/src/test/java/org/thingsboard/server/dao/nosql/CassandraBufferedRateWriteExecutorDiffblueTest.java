...
public class CassandraBufferedRateWriteExecutorDiffblueTest {
   ...
   * <p>
   * Method under test: {@link CassandraBufferedRateWriteExecutor#printStats()}
   */
  @Test
  public void testPrintStats() {
    ...
    // Act
    (new CassandraBufferedRateWriteExecutor(1, -1, 1L, 1, 1, 1L, true, 1, statsFactory, entityService,
        new DefaultRateLimitService(mock(TenantProfileProvider.class), mock(NotificationRuleProcessor.class), 1, 3)))
        .printStats();

    // Assert
    verify(statsFactory).createGauge(eq("rateExecutor.currBufferWrite"), isA(AtomicInteger.class), isA(String[].class));
  ...
  public void testPrintStats3() {
    ...
    // Act
    (new CassandraBufferedRateWriteExecutor(1, 0, 1L, 1, 1, Long.MIN_VALUE, true, 1, statsFactory, entityService,
        new DefaultRateLimitService(mock(TenantProfileProvider.class), mock(NotificationRuleProcessor.class), 1, 3)))
        .printStats();
   ...
   * <p>
   * Method under test: {@link CassandraBufferedRateWriteExecutor#stop()}
   */
  @Test
   ...
   * Test {@link CassandraBufferedRateWriteExecutor#stop()}.
   * <ul>
   *   <li>Given {@link CumulativeCounter} {@link Counter#increment()} does
   * nothing.</li>
   *   <li>Then calls {@link Counter#increment()}.</li>
   * </ul>
   * <p>
   * Method under test: {@link CassandraBufferedRateWriteExecutor#stop()}
   */
  @Test
  public void testStop_givenCumulativeCounterIncrementDoesNothing_thenCallsIncrement() {
    //   Diffblue Cover was unable to create a Spring-specific test for this Spring method.

    // Arrange
    CumulativeCounter micrometerCounter = mock(CumulativeCounter.class);
    doNothing().when(micrometerCounter).increment();
    ...
    // Act
    (new CassandraBufferedRateWriteExecutor(1, 1, 0L, 1, 1, 1L, true, -1, statsFactory, entityService,
        new DefaultRateLimitService(mock(TenantProfileProvider.class), mock(NotificationRuleProcessor.class), 1, 3)))
        .stop();

    // Assert that nothing has changed
    verify(micrometerCounter).increment();
    verify(statsFactory).createGauge(eq("rateExecutor.currBufferWrite"), isA(AtomicInteger.class), isA(String[].class));
    verify(statsFactory, atLeast(1)).createStatsCounter(eq("rateExecutor"), Mockito.<String>any(), isA(String[].class));
  }
   ...
   * <p>
   * Method under test: {@link CassandraBufferedRateWriteExecutor#create()}
   */
  @Test
  ...
  public void testCreate3() {
    ...
    when(statsFactory.createGauge(Mockito.<String>any(), Mockito.<AtomicInteger>any(), isA(String[].class)))
        .thenReturn(new AtomicInteger(1));
    when(statsFactory.createStatsCounter(Mockito.<String>any(), Mockito.<String>any(), isA(String[].class)))
        .thenReturn(new StatsCounter(new AtomicInteger(1), null, "Name"));
    BaseEntityService entityService = new BaseEntityService();
