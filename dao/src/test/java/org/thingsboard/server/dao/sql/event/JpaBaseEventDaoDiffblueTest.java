...
public class JpaBaseEventDaoDiffblueTest {
  ...
  public void testSaveAsync_thenThrowRuntimeException() {
    // Arrange
   ...
   *
   * <ul>
   *   <li>Given {@link ErrorEventRepository} {@link ErrorEventRepository#findEvents(UUID, UUID,
   *       Long, Long, Pageable)} throw {@link RuntimeException#RuntimeException()}.
   * </ul>
   *
   * <p>Method under test: {@link JpaBaseEventDao#findEvents(UUID, UUID, EventType, TimePageLink)}
   */
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"PageData JpaBaseEventDao.findEvents(UUID, UUID, EventType, TimePageLink)"})
  public void testFindEvents_givenErrorEventRepositoryFindEventsThrowRuntimeException() {
    // Arrange
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"PageData JpaBaseEventDao.findEvents(UUID, UUID, EventType, TimePageLink)"})
  public void testFindEvents_thenThrowIllegalArgumentException() {
    // Arrange
    when(errorEventRepository.findEvents(
            Mockito.<UUID>any(),
            Mockito.<UUID>any(),
   ...
   *
   * <ul>
   *   <li>Then calls {@link RuleNodeDebugEventRepository#findEvents(UUID, UUID, Long, Long, String,
   ...
   *
   * <ul>
   *   <li>When {@link UUID#UUID(long, long)} with four and four.
   * </ul>
   *
   * <p>Method under test: {@link JpaBaseEventDao#removeEvents(UUID, UUID, Long, Long)}
   */
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"void JpaBaseEventDao.removeEvents(UUID, UUID, Long, Long)"})
  public void testRemoveEventsWithTenantIdEntityIdStartTimeEndTime_whenUuidWithFourAndFour() {
    // Arrange
    doNothing()
        .when(lifecycleEventRepository)
        .removeEvents(
    ...
    // Act
    jpaBaseEventDao.removeEvents(new UUID(4L, 4L), ModelConstants.NULL_UUID, 1L, 1L);
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"List JpaBaseEventDao.findLatestEvents(UUID, UUID, EventType, int)"})
  public void testFindLatestEvents_thenThrowIllegalArgumentException() {
    // Arrange
    when(errorEventRepository.findLatestEvents(Mockito.<UUID>any(), Mockito.<UUID>any(), anyInt()))
        .thenThrow(new IllegalArgumentException());
    ...
    assertThrows(
        ...
        () ->
            jpaBaseEventDao.findLatestEvents(
                new UUID(1L, 1L), ModelConstants.NULL_UUID, EventType.ERROR, 1));
    verify(errorEventRepository).findLatestEvents(isA(UUID.class), isA(UUID.class), eq(1));
  }
  ...
  public void testCleanupEventsWithRegularEventExpTsDebugEventExpTsCleanupDb4() {
    // Arrange
    when(eventPartitionConfiguration.getPartitionSizeInMs(Mockito.<EventType>any())).thenReturn(4L);
    when(sqlPartitioningRepository.dropPartitionsBefore(
            Mockito.<String>any(), anyLong(), anyLong()))
        .thenReturn(1L);
    ...
    verify(eventPartitionConfiguration, atLeast(1)).getPartitionSizeInMs(Mockito.<EventType>any());
    verify(sqlPartitioningRepository, atLeast(1))
        .dropPartitionsBefore(Mockito.<String>any(), eq(1L), eq(4L));
  }
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"void JpaBaseEventDao.cleanupEvents(long, long, boolean)"})
  public void testCleanupEventsWithRegularEventExpTsDebugEventExpTsCleanupDb10() {
    // Arrange
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"void JpaBaseEventDao.cleanupEvents(long, long, boolean)"})
  public void testCleanupEventsWithRegularEventExpTsDebugEventExpTsCleanupDb_thenDoesNotThrow() {
    // Arrange, Act and Assert
    jpaBaseEventDao.cleanupEvents(0L, 0L, false);
  }
}
