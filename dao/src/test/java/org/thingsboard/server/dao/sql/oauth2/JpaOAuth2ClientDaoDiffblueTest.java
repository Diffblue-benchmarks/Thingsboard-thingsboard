...
public class JpaOAuth2ClientDaoDiffblueTest {
  ...
  public void testFindByTenantId_givenArrayListAddAllArrayList_thenReturnDataSizeIsOne() {
    // Arrange
    ...
    JpaOAuth2ClientDao jpaOAuth2ClientDao = new JpaOAuth2ClientDao(repository);
    ...
    PageData<OAuth2Client> actualFindByTenantIdResult =
        jpaOAuth2ClientDao.findByTenantId(tenantId, pageLink);
    ...
    verify(repository).findByTenantId(isA(UUID.class), eq("Text Search"), isA(Pageable.class));
    List<OAuth2Client> data = actualFindByTenantIdResult.getData();
    assertEquals(1, data.size());
    OAuth2Client getResult = data.get(0);
