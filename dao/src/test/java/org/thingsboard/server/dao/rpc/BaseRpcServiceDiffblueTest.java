...
public class BaseRpcServiceDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Then calls {@link RpcRepository#flush()}.
   * </ul>
   *
   * <p>Method under test: {@link BaseRpcService#deleteAllRpcByTenantId(TenantId)}
   */
  ...
  public void testDeleteAllRpcByTenantId_thenCallsFlush() {
    ...
    doNothing().when(rpcRepository).flush();
    doNothing().when(rpcRepository).deleteById(Mockito.<UUID>any());
    when(rpcRepository.findAllByTenantId(Mockito.<UUID>any(), Mockito.<Pageable>any()))
        .thenReturn(pageImpl);
    JpaRpcDao rpcDao = new JpaRpcDao(rpcRepository);

    // Act
    new BaseRpcService(rpcDao).deleteAllRpcByTenantId(ModelConstants.SYSTEM_TENANT);

    // Assert
    verify(rpcRepository).flush();
    verify(rpcRepository).deleteById(isA(UUID.class));
  ...
  public void testDeleteByTenantId_thenCallsRemoveById() {
    // Arrange
    Rpc rpc = new Rpc();
    rpc.setId(new RpcId(ModelConstants.NULL_UUID));
  ...
  public void testFindAllByDeviceIdAndStatus_givenBy_created_time_desc() {
    // Arrange
    PageData<Rpc> emptyPageDataResult = PageData.emptyPageData();
  ...
  public void testFindAllByDeviceIdAndStatus_givenSortOrderGetPropertyReturnNull() {
    // Arrange
    PageData<Rpc> emptyPageDataResult = PageData.emptyPageData();
    when(rpcDao.findAllByDeviceIdAndStatus(
            Mockito.<TenantId>any(),
  ...
  public void testFindAllByDeviceId_givenBy_created_time_desc() {
    // Arrange
    PageData<Rpc> emptyPageDataResult = PageData.emptyPageData();
  ...
  @ManagedByDiffblue
  @MethodsUnderTest({"PageData BaseRpcService.findAllByDeviceId(TenantId, DeviceId, PageLink)"})
  public void testFindAllByDeviceId_givenSortOrderGetPropertyReturnNull_thenCallsGetProperty() {
    // Arrange
    PageData<Rpc> emptyPageDataResult = PageData.emptyPageData();
    when(rpcDao.findAllByDeviceId(
            Mockito.<TenantId>any(), Mockito.<DeviceId>any(), Mockito.<PageLink>any()))
