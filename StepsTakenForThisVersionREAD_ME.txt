...
#second comit
(13) added PITest plugin for mutation tests and aggregate module (had to remove "@{argLine}" in surefire-maven-plugin args line to work with PITest plugin. To produce jacoco reports this needs adding in and PITest plugin commenting out)

#Third comit
(14) commented out PITest plugin per module and in main POM, added back in "@{argLine}" in surefire-maven-plugin args line. Used dcover refactor to make Cover suggested changes to modules with a refactor.yaml file - needed to comment out the maven-install-plugin plugin in main POM for refactor to work.
