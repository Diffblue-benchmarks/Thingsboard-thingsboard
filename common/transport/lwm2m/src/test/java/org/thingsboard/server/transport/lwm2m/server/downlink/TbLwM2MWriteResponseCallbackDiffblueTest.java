...
class TbLwM2MWriteResponseCallbackDiffblueTest {
  ...
  void testOnSuccessWithWriteRequestWriteResponse_thenCallsLog() {
    ...
    doNothing().when(handler)
        .onWriteResponseOk(Mockito.<LwM2mClient>any(), Mockito.<String>any(), Mockito.<WriteRequest>any(), anyInt());
    ...
    TbLwM2MWriteResponseCallback tbLwM2MWriteResponseCallback = new TbLwM2MWriteResponseCallback(handler, logService,
    ...
    verify(handler).onWriteResponseOk(isA(LwM2mClient.class), eq("42"), isA(WriteRequest.class), eq(405));
  }
}
