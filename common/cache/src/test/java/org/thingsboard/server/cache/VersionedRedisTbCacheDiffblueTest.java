...
class VersionedRedisTbCacheDiffblueTest {
  ...
  @Test
  @DisplayName(
      "Test put(VersionedCacheKey, Serializable) with 'DeviceCacheKey', 'Device'; when DeviceId(UUID) with id is randomUUID; then does not throw")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void VersionedRedisTbCache.put(VersionedCacheKey, Serializable)"})
  void testPutWithDeviceCacheKeyDevice_whenDeviceIdWithIdIsRandomUUID_thenDoesNotThrow() {
    // Arrange
    TBRedisClusterConfiguration configuration = new TBRedisClusterConfiguration();
    CacheSpecsMap cacheSpecsMap = new CacheSpecsMap();
  ...
  void testPutWithDeviceCacheKeyDevice_whenNull_thenDoesNotThrow() {
    ...
    // Act and Assert
    assertDoesNotThrow(
        () -> deviceRedisCache.put(new DeviceCacheKey(new DeviceId(UUID.randomUUID())), null));
  }
