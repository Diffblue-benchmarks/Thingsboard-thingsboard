...
class AbstractTbQueueTemplateDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Then return array of {@code byte} with {@code x} and {@code O}.
   * </ul>
   *
   * <p>Method under test: {@link AbstractTbQueueTemplate#uuidToBytes(UUID)}
   */
  @Test
  ...
  void testUuidToBytes_thenReturnArrayOfByteWithXAndO() {
    ...
    assertArrayEquals(
        new byte[] {
          'x', 'O', '9', 'L', 'B', -74, 'C', 'Z', -104, '<', -73, -66, -1, '\'', -124, -7
        },
        abstractTbQueueTemplate.uuidToBytes(
