...
class AttributesDeleteNodeCallbackDiffblueTest {
   ...
   * <ul>
   *   <li>Then calls
   * {@link TbContext#attributesDeletedActionMsg(EntityId, RuleNodeId, String, List)}.</li>
   * </ul>
   * <p>
   * Method under test: {@link AttributesDeleteNodeCallback#onSuccess(Void)}
   */
  @Test
  @DisplayName("Test onSuccess(Void) with 'Void'; then calls attributesDeletedActionMsg(EntityId, RuleNodeId, String, List)")
  void testOnSuccessWithVoid_thenCallsAttributesDeletedActionMsg() {
    ...
    when(ctx.attributesDeletedActionMsg(Mockito.<EntityId>any(), Mockito.<RuleNodeId>any(), Mockito.<String>any(),
        Mockito.<List<String>>any())).thenReturn(buildResult);
    ...
    // Act
    (new AttributesDeleteNodeCallback(ctx, msg, "Scope", new ArrayList<>())).onSuccess(null);

    // Assert
    verify(ctx).attributesDeletedActionMsg(isNull(), isA(RuleNodeId.class), eq("Scope"), isA(List.class));
