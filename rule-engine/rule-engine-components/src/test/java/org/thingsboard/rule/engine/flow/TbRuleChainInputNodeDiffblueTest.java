...
class TbRuleChainInputNodeDiffblueTest {
   ...
   *
   * <ul>
   *   <li>Given {@link TbRuleChainInputNode} (default constructor).
   *   <li>When {@link TbContext} {@link TbContext#input(TbMsg, RuleChainId)} does nothing.
   *   <li>Then calls {@link TbContext#input(TbMsg, RuleChainId)}.
   * </ul>
   *
  ...
  @Test
  @DisplayName(
      "Test onMsg(TbContext, TbMsg); given TbRuleChainInputNode (default constructor); when TbContext input(TbMsg, RuleChainId) does nothing; then calls input(TbMsg, RuleChainId)")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void TbRuleChainInputNode.onMsg(TbContext, TbMsg)"})
  void testOnMsg_givenTbRuleChainInputNode_whenTbContextInputDoesNothing_thenCallsInput()
      throws TbNodeException {
    // Arrange
    TbRuleChainInputNode tbRuleChainInputNode = new TbRuleChainInputNode();
    ...
    // Act
    tbRuleChainInputNode.onMsg(ctx, msg);
  ...
  @Test
  @DisplayName(
      "Test upgrade(int, JsonNode); given TbRuleChainInputNode (default constructor); when one; then return Second is valueOf ten")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({
    "org.thingsboard.server.common.data.util.TbPair TbRuleChainInputNode.upgrade(int, JsonNode)"
  })
  void testUpgrade_givenTbRuleChainInputNode_whenOne_thenReturnSecondIsValueOfTen()
      throws TbNodeException {
    // Arrange
