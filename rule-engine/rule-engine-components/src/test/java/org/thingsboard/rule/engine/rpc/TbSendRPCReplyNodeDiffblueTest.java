...
class TbSendRPCReplyNodeDiffblueTest {
  ...
  void testInit_whenPOJONodeWithVIsNull_thenDoesNotThrow() throws TbNodeException {
    ...
    // Act and Assert
    assertDoesNotThrow(
        () -> tbSendRPCReplyNode.init(ctx, new TbNodeConfiguration(new POJONode(null))));
  }
   ...
   *
   * <ul>
   *   <li>When {@link POJONode#POJONode(Object)} with v is {@link TbSendRpcReplyNodeConfiguration}
   *       (default constructor).
   *   <li>Then does not throw.
   * </ul>
   *
  ...
  @Test
  @DisplayName(
      "Test init(TbContext, TbNodeConfiguration); when POJONode(Object) with v is TbSendRpcReplyNodeConfiguration (default constructor); then does not throw")
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void TbSendRPCReplyNode.init(TbContext, TbNodeConfiguration)"})
  void testInit_whenPOJONodeWithVIsTbSendRpcReplyNodeConfiguration_thenDoesNotThrow()
      throws TbNodeException {
    // Arrange
    TbSendRPCReplyNode tbSendRPCReplyNode = new TbSendRPCReplyNode();
    TbContext ctx = mock(TbContext.class);
    ...
    assertDoesNotThrow(
        () ->
            tbSendRPCReplyNode.init(
                ctx, new TbNodeConfiguration(new POJONode(new TbSendRpcReplyNodeConfiguration()))));
  }
  ...
  void testInit_whenTbNodeConfigurationWithDataIsNull_thenDoesNotThrow() throws TbNodeException {
    ...
    // Act and Assert
    assertDoesNotThrow(() -> tbSendRPCReplyNode.init(ctx, new TbNodeConfiguration(null)));
  }

  /**
   * Test {@link TbSendRPCReplyNode#onMsg(TbContext, TbMsg)}.
   *
   * <ul>
   ...
   * </ul>
   *
   * <p>Method under test: {@link TbSendRPCReplyNode#onMsg(TbContext, TbMsg)}
   */
  @Test
  @DisplayName(
  ...
  @Tag("ContributionFromDiffblue")
  @ManagedByDiffblue
  @MethodsUnderTest({"void TbSendRPCReplyNode.onMsg(TbContext, TbMsg)"})
  void testOnMsg_givenRuntimeException_thenThrowRuntimeException() {
    ...
    // Act and Assert
    assertThrows(RuntimeException.class, () -> tbSendRPCReplyNode.onMsg(ctx, msg));
