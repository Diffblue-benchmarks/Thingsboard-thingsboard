...
class EntitiesByNameAndTypeLoaderDiffblueTest {
  ...
  void testFindEntityId() {
    ...
    when(ctx.getUserService()).thenReturn(new UserServiceImpl(userDao, userCredentialsDao, userAuthSettingsDao,
  ...
  void testFindEntityId_whenTbContext_thenThrowIllegalStateException() {
    ...
    assertThrows(IllegalStateException.class,
        () -> EntitiesByNameAndTypeLoader.findEntityId(mock(TbContext.class), EntityType.TENANT, "Entity Name"));
  }
